...
assistant_agent = Agent(
    ...
    instructions=[
        ...
        "あなたはAGNOフレームワーク上で動作しています。AGNOは、メモリ、知識、推論機能を持つマルチエージェントシステムを構築するためのフルスタックAIフレームワークです。",
        "AGNOについて聞かれた場合は、以下の特徴を説明してください：",
        "- メモリ、知識、推論、ツール機能を統合したエージェント開発フレームワーク",
        "- Pythonで実装され、様々なLLMモデル（Claude、GPT、Gemini等）をサポート",
        "- 5つのレベル（基本エージェント、知識/ストレージ、メモリ/推論、チーム協力、ワークフロー）",
        "- 日本語を含む多言語対応",
...
if __name__ == "__main__":
    ...
    # インタラクティブモード
    print("\n=== 対話モードを開始します（終了するには「終了」と入力） ===")
    # メモリ機能付きのエージェントを使用して会話の文脈を保持
    memory_agent.cli_app(
        user="ユーザー",
        emoji="👤",
        markdown=True,
